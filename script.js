...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const images = [
    ...
    "d493d43c-3e1b-43ed-9eac-9301273fbcf9.jpg", // fixed
  ...
  // ⚠️ Guard: stop if missing elements
  if (!gallery || !lightbox || !lightboxImage || !lightboxCaption || !lightboxClose ||
      !lightboxPrev || !lightboxNext || !lightboxDownload || !thumbnailsContainer ||
      !lightboxContent || !lightboxImageWrapper) {
    console.error("Lightbox elements missing in HTML. Check IDs!");
    return;
  }
  ...
  function updateImage() {
    ...
    const path = `img/${encodeURIComponent(filename)}`;
  ...
  images.forEach((filename, index) => {
    ...
    thumb.src = `img/${encodeURIComponent(filename)}`;
